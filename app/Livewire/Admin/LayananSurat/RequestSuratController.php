...
class RequestSuratController extends Controller
{
    ...
    public function download($id)
    {
        ...
        // Auto-regenerate jika file PDF ini adalah hasil generate sistem (bukan upload manual)
        // agar perubahan pada template surat langsung ter-update di file PDF saat didownload
        $isAutogenerated = preg_match('/surat\/\d{8}_\d+_.+\.pdf$/i', $pengajuanSurat->file_pdf);
        if ($isAutogenerated) {
            try {
                $judul = $pengajuanSurat->jenisSurat->nama_surat ?? 'Surat Keterangan';
                $isPdf = true;
                ...
                \Log::warning('Gagal update auto-generated PDF pada download: ' . $e->getMessage());
            }
    ...
    private function notifyUser(int $idPenduduk, string $judul, string $pesan): void
    {
        ...
        // Kirim push notification real-time ke HP masyarakat via FCM
        try {
