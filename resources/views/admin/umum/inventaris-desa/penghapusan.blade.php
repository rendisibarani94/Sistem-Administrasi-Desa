<div>
    <x-slot:judul>
        Data Penghapusan Inventaris Desa
    </x-slot:judul>

    {{-- Full Page Container --}}
    <div class="mx-4">
        <div class="flex justify-between">
            <h1 class="text-3xl font-semibold mt-6 mb-6">Penghapusan Inventaris Desa</h1>
        </div>
        ...
        <div class="flex flex-wrap justify-between items-center border-2 border-gray-300 rounded-sm my-6 p-4 gap-4 sm:justify-between">
            <!-- Button -->
            <a href="{{ route('InventarisDesa') }}" class="cursor-pointer bg-sky-700 hover:bg-sky-800 text-white focus:ring-2 focus:outline-none focus:ring-sky-700 font-bold py-2 px-4 rounded flex items-center space-x-2 w-full sm:w-auto">
                ...
                <span>Data Inventaris Desa</span>
            </a>
            ...
            <div class="relative w-full sm:w-72">
                ...
                <input type="search" type="search" wire:model.live="search" autocomplete="off" class="block w-full sm:w-72 p-3 ps-10 text-sm text-gray-900 border border-gray-400 rounded-lg bg-gray-50 focus:ring-sky-500 focus:border-sky-500" placeholder="Cari Aparatuwr Desa" required />
            </div>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 table-fixed" id="dataTable">
                <thead class="bg-gray-100">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-700 uppercase tracking-wider border-b-3 border-gray-500 cursor-pointer">
                            Jumlah Inventaris Yang Dijual
                        </th>
                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-700 uppercase tracking-wider border-b-3 border-gray-500 cursor-pointer">
                            Jumlah Inventaris Yang Disumbangkan
                        </th>
                        <th scope="col" class="px-6 py-3 text-center text-xs font-medium text-gray-700 uppercase tracking-wider border-b-3 border-gray-500 cursor-pointer">
                            Tanggal Penghapusan
                        </th>
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                    ...
                    <tr class="hover:bg-gray-300 transition duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center font-semibold">
                            {{ $item->jumlah_disumbangkan  }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center font-semibold">
                            {{ \Carbon\Carbon::parse($item->tanggal_penghapusan)->locale('id')->translatedFormat('d F Y') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap font-semibold flex justify-center space-x-4">
                            <!-- Edit Button -->
                            <a href="{{ route('InventarisDesa.deleted.edit', $item->id_inventaris_desa) }}" class="text-green-600 hover:text-green-900 font-medium transition flex items-center space-x-1.5 cursor-pointer">
