<div>
    ...
    <div class="mx-4">
        <div class="flex justify-between">
            <h1 class="text-3xl font-semibold mt-6 mb-6">Keputusan Kepala Desa</h1>
        </div>
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ms-1 text-sm font-semibold text-gray-500 md:ms-2">Keputusan Kepala Desa</span>
                        </div>
                    </li>
                </ol>
            </nav>

            <a href="/A2" class="cursor-pointer bg-sky-700 hover:bg-sky-800 text-white focus:ring-2 focus:outline-none focus:ring-sky-600 font-bold py-2 px-4 rounded flex items-center space-x-2 w-full sm:w-auto">
                <svg class="w-6 h-6 text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
                    <path fill-rule="evenodd" d="M8 3a2 2 0 0 0-2 2v3h12V5a2 2 0 0 0-2-2H8Zm-3 7a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h1v-4a1 1 0 0 1 1-1h10a1 1 0 0 1 1 1v4h1a2 2 0 0 0 2-2v-5a2 2 0 0 0-2-2H5Zm4 11a1 1 0 0 1-1-1v-4h8v4a1 1 0 0 1-1 1H9Z" clip-rule="evenodd" />
                </svg>
                <span>Cetak Laporan Keputusan Kepala Desa</span>
            </a>
        </div>
        ...
        <div class="flex flex-wrap justify-between items-center border-2 border-gray-300 rounded-sm my-6 p-4 gap-4 sm:justify-between">
            <!-- Button -->
            <a href="{{ route('keputusanKepalaDesa.create') }}" class="cursor-pointer bg-sky-700 hover:bg-sky-800 text-white focus:ring-2 focus:outline-none focus:ring-sky-600 font-bold py-2 px-4 rounded flex items-center space-x-2 w-full sm:w-auto">
                ...
                <span>Tambah Data Keputusan Kepala Desa</span>
            </a>
            ...
            <div class="relative w-full sm:w-72">
                ...
                <input type="search" type="search" wire:model.live="search" autocomplete="off" class="block w-full sm:w-72 p-3 ps-10 text-sm text-gray-900 border border-gray-400 rounded-lg bg-gray-50 focus:ring-sky-500 focus:border-sky-500" placeholder="Cari Keputusan Kepala Desa" required />
            </div>
        </div>
        ...
        <div class="relative overflow-x-auto shadow-md mt-4 border-b-2 border-gray-300">
            <table class="min-w-full divide-y divide-gray-200 table-fixed" id="dataTable">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                    ...
                    <tr class="hover:bg-gray-300 transition duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center font-semibold">
                            {{ \Illuminate\Support\Str::limit($item->tentang, 20, '...') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center font-semibold">
                            {{ \Carbon\Carbon::parse($item->tanggal_dilaporkan)->locale('id')->translatedFormat('d F Y') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center font-semibold">
                            @if($item->tanggal_dilaporkan)
                            {{ \Carbon\Carbon::parse($item->tanggal_dilaporkan)->locale('id')->translatedFormat('d F Y') }}
                            @else
                            {{ "Belum Dilaporkan" }}
                            @endif
                        ...
                        <td class="px-6 py-4 whitespace-nowrap font-semibold flex justify-center space-x-4">
                            <!-- Edit Button -->
                            <a href="{{ route('keputusanKepalaDesa.edit', $item->id_keputusan_kepala_desa) }}" class="text-green-600 hover:text-green-900 font-medium transition flex items-center space-x-1.5 cursor-pointer">
                            ...
                            <!-- Delete Button -->
                            <a wire:click="confirmDelete({{ $item->id_keputusan_kepala_desa }})" class="text-red-600 hover:text-red-900 font-medium transition flex items-center space-x-1.5 cursor-pointer">
            ...
            <div class="mt-4">
                {{ $keputusanKepalaDesaData->links('vendor.pagination.tailwind') }}
            </div>
        </div>
    </div>
