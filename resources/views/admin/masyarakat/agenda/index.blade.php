<div>
    ...
    <div class="mx-4">
        <div class="flex justify-between">
            <h1 class="text-3xl font-semibold mt-6 mb-6">Data Agenda Desa</h1>
        </div>
            ...
            <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li aria-current="page">
                    <div class="flex items-center">
                        ...
                        <span class="ms-1 text-sm font-semibold text-gray-500 md:ms-2">Agenda Desa</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <div class="flex flex-wrap justify-between items-center border-2 border-gray-300 rounded-sm my-6 p-4 gap-4 sm:justify-between">
            <!-- Button -->
            <a href="{{ route('admin.agenda.create') }}" class="cursor-pointer bg-sky-700 hover:bg-sky-800 text-white focus:ring-2 focus:outline-none focus:ring-sky-600 font-bold py-2 px-4 rounded flex items-center space-x-2 w-full sm:w-auto">
            ...
            <div class="relative w-full sm:w-72">
                ...
                <input type="search" type="search" wire:model.live="search" autocomplete="off" class="block w-full sm:w-72 p-3 ps-10 text-sm text-gray-900 border border-gray-400 rounded-lg bg-gray-50 focus:ring-sky-500 focus:border-sky-500" placeholder="Cari Agenda" required />
            </div>
        </div>
        ...
        <div class="relative overflow-x-auto shadow-md mt-4 border-b-2 border-gray-300">
            <table class="min-w-full divide-y divide-gray-200 table-fixed" id="dataTable">
                ...
                <tbody class="bg-white divide-y divide-gray-200" id="tableBody">
                    @foreach ($agendaData as $index => $item)
                    <tr class="hover:bg-gray-300 transition duration-200">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center font-semibold">
                            {{ \Illuminate\Support\Str::limit($item->tujuan_agenda, 25) }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-center font-semibold">
                            {{ \Carbon\Carbon::parse($item->tanggal_agenda)->locale('id')->translatedFormat('d F Y') }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap font-semibold flex justify-center space-x-4">
                            <!-- Edit Button -->
                            <a href="{{ route('admin.agenda.edit', $item->id_agenda) }}" class="text-green-600 hover:text-green-900 font-medium transition flex items-center space-x-1.5 cursor-pointer">
                            ...
                            <a wire:click="confirmDelete({{ $item->id_agenda }})" class="text-red-600 hover:text-red-900 font-medium transition flex items-center space-x-1.5 cursor-pointer">
            ...
            <div class="mt-4">
                {{ $agendaData->links('vendor.pagination.tailwind') }}
            </div>
        </div>
    </div>
</div>
