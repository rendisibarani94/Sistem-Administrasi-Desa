<div>
    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between">
            <h1 class="text-3xl font-semibold mt-6 mb-6">Data Jabatan Aparatur Desa</h1>
        </div>
    </div>
    ...
    <div class="flex flex-wrap justify-between items-center border-2 border-gray-300 rounded-sm my-6 p-4 gap-4 sm:justify-between">
        ...
        <button type="button" data-modal-target="store-modal" data-modal-toggle="store-modal" class="cursor-pointer bg-teal-700 text-white font-bold py-2 px-4 rounded flex items-center space-x-2 w-full sm:w-auto">
            ...
            <span>Tambah Data Jabatan Aparatur Desa</span>
        </button>
        ...
        <div class="relative w-full sm:w-72">
            ...
            <input type="search" type="search" wire:model.live="search" autocomplete="off" class="block w-full sm:w-72 p-3 ps-10 text-sm text-gray-900 border border-gray-400 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Cari Jenis Peraturan" required />
        </div>
                ...
                <div class="relative overflow-x-auto shadow-md mt-4 border-b-2 border-gray-300">
                    <table class="w-full text-sm text-left rtl:text-right text-gray-500 table-fixed" id="dataTable">
                        ...
                        <tbody id="tableBody">
                            @foreach ($jabatanData as $index => $item)
                            <tr class="page-row bg-white hover:bg-gray-300 transition duration-200 border-b-2 border-gray-300 @if($loop->index >= 10) hidden @endif" data-index="{{ $index }}">
                                ...
                                <td class="px-6 py-4 font-semibold flex space-x-4 justify-center">
                                    ...
                                    <!-- Edit Button -->
                                    <button wire:click="loadEditData({{ $item->id_jabatan }})" class="text-blue-600 hover:text-blue-800 hover:font-bold font-medium transition rounded-sm duration-200 flex items-center space-x-1.5 cursor-pointer">
                                    ...
                                    <!-- Delete Button -->
                                    <button type="button" wire:click="confirmDelete({{ $item->id_jabatan }})" class="text-red-600 hover:text-red-800 hover:font-bold font-medium transition duration-200 flex items-center space-x-1.5 cursor-pointer">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                    ...
                    <div class="mt-4">
                        {{ $jabatanData->links('vendor.pagination.tailwind') }}
                    </div>
                </div>
    </div>
                    ...
                    <form wire:submit.prevent="store" class="p-4 md:p-5">
                        <div class="grid gap-4 mb-4 grid-cols-2">
                            <div class="col-span-2">
                                ...
                                <input type="text" wire:model="jabatan" id="jabatan" autocomplete="off" class="bg-gray-50 border-2 border-gray-400 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="Jabatan" required>
                            </div>
                        </div>
        ...
        <!-- Edit modal -->
        <div id="edit-modal" tabindex="-1" aria-hidden="true" class="{{ $this->jabatanId ? '' : 'hidden' }} flex inset-0 z-40 bg-black/50 transition-opacity duration-300 ease-out overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
            <div class="relative p-4 w-full max-w-xl max-h-full transform transition-all duration-300 ease-out scale-95">
                <!-- Modal content -->
                <div class="relative bg-white rounded-lg shadow-md border-2 border-gray-500">
                    <!-- Modal header -->
