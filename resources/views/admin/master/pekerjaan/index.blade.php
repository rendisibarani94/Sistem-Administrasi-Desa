<div>
    ...
    <div class="bg-white rounded-lg shadow-md mx-4">
                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    ...
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ms-1 text-sm font-semibold text-gray-500 md:ms-2">Pekerjaan</span>
                        </div>
                    </li>
                </ol>
            </nav>
        ...
        <div class="flex flex-wrap justify-between items-center border-2 border-gray-300 rounded-sm my-6 p-4 gap-4 sm:justify-between">
            ...
            <div class="relative w-full sm:w-72">
                ...
                <input type="search" type="search" wire:model.live="search" autocomplete="off" class="block w-full sm:w-72 p-3 ps-10 text-sm text-gray-900 border border-gray-400 rounded-lg bg-gray-50 focus:ring-sky-700 focus:border-sky-700" placeholder="Cari Data" required />
            </div>
        </div>
                ...
                <tbody id="tableBody">
                    ...
                    <tr class="page-row bg-white hover:bg-gray-300 transition duration-200 border-b-2 border-gray-300 @if($loop->index >= 10) hidden @endif" data-index="{{ $index }}">
                        ...
                        <td class="px-6 py-4 font-semibold flex space-x-4 justify-center">
                            ...
                            <!-- Edit Button -->
                            <button wire:click="loadPekerjaan({{ $item->id_pekerjaan }})" class="text-green-600 hover:text-green-800 hover:font-bold font-medium transition rounded-sm duration-200 flex items-center space-x-1.5 cursor-pointer">
                            ...
                            <!-- Delete Button -->
                            <button type="button" wire:click="confirmDelete({{ $item->id_pekerjaan }})" class="text-red-600 hover:text-red-800 hover:font-bold font-medium transition duration-200 flex items-center space-x-1.5 cursor-pointer">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5">
                ...
                <form wire:submit.prevent="store" class="p-4 md:p-5">
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2">
                            ...
                            <input type="text" wire:model="pekerjaan" id="pekerjaan" autocomplete="off" class="bg-gray-50 border-2 border-gray-400 text-gray-900 text-sm rounded-lg focus:ring-sky-700 focus:border-sky-700 block w-full p-2.5" placeholder="Nama Pekerjaan" required>
                        </div>
                    </div>
                    <button type="submit" class="font-semibold text-white inline-flex items-center bg-sky-700 hover:bg-sky-900 focus:ring-4 focus:outline-none focus:ring-sky-700 rounded-lg ml-auto text-sm px-5 py-2.5 text-center cursor-pointer">
                        Tambahkan Pekerjaan
                    </button>
                </form>
            </div>
    ...
    <!-- Edit modal -->
    <div id="edit-modal" tabindex="-1" aria-hidden="true" class="{{ $this->pekerjaanId ? '' : 'hidden' }} flex inset-0 z-40 bg-black/50 transition-opacity duration-300 ease-out overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
        <div class="relative p-4 w-full max-w-xl max-h-full transform transition-all duration-300 ease-out scale-95">
            <!-- Modal content -->
            <div class="relative bg-white rounded-lg shadow-md border-2 border-gray-500">
                <!-- Modal header -->
